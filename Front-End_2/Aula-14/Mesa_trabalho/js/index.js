...
function renderizarDadosUsuario(dados) {
    ...
    let capturaElementoPai = document.querySelector('.card');
    // Constrói um card com os dados do usuário dentro de tarjetaDados
    const estruturaCard = `<img src="${dados.imagem.grande}" class="card-img-top" alt="...">
        <div class="card-body">
            <h3 class="card-title">${dados.nome.primeiro} ${dados.nome.utlimo}</h5>
            <p class="card-text">${dados.email}</p>
        </div>`;

    let novoCard = document.createElement("div");
    novoCard.setAttribute('class', "card-content");

    novoCard.innerHTML = estruturaCard;
